# # # # # Special Notes While Coding
# # # # 1. When writing the reqInLetters, do NOT use the spacebar.
...
class Enemy:
    global enemy
    enemy = []
    def __init__(self, name, hp, reqInLetters, reqInNumber):
        self.name = name
        self.hp = hp
        self.reqInLetters = reqInLetters
        self.reqInNumber = reqInNumber
    def add(self):
        enemy.append(self.name)
    def killreq(self):
        if (Kills < bb.reqInNumber):
            enemy = ["Zombie"]
        elif (Kills >= bb.reqInNumber and Kills < cc.reqInNumber):
            enemy = ["Zombie", "Skeleton"]
        elif (Kills >= cc.reqInNumber and Kills < dd.reqInNumber):
            enemy = ["Zombie", "Skeleton", "Assasin"]
        else:
            enemy = ["Zombie", "Skeleton", "Assasin", "Dungeon Warrior"]
...
aa = Enemy("Zombie", 70, "Zero", 0)
aa.add()
bb = Enemy("Skeleton", 100, "Five", 5)
bb.add()
cc = Enemy("Assassin", 150, "Ten", 10)
cc.add()
dd = Enemy("Dungeon Warrior", 250, "TwentyFive", 25)
dd.add()
...
def killReq():
    ...
    if (len(enemy) == 1):
        ...
        eencountered = enemy[r.randint(0, 1)]
    elif (len(enemy) == 3):
        eencountered = enemy[r.randint(0, 2)]
    else:
        eencountered = enemy[r.randint(0, 3)]
# # # 2. Turns
def enemyHP():
    global eehp                                     # enemy encountered hp
    if eencountered == "Zombie":
        eehp = 70
    if eencountered == "Skeleton":
        eehp = 100
    if eencountered == "Assassin":
        eehp = 150
    else:
        eehp = 250
# # # 3. Turns
    ...
    while (True):
        ...
        if (a == "Attack"):
            ...
            if (Chance100 <= crithit.perc):
                    ...
                    eehp = eehp - int(CritHit)
                    print("A critical hit!")
                    print(f"You dealt {int(CritHit)} DMG to the {eencountered}! It now has {eehp} HP left!")
                    print(f"The {eencountered} retaliated, dealing {EnemyDMG} DMG to you! You now have {PlayerHP} HP left!")
                    if (PlayerHP <= 0 and eehp <= 0):
                ...
                PlayerHP = PlayerHP - EnemyDMG
                eehp = eehp - PlayerDMG
                print(f"You dealt {PlayerDMG} DMG to the {eencountered}! It now has {eehp} HP left!")
                print(f"The {eencountered} retaliated, dealing {EnemyDMG} DMG to you! You now have {PlayerHP} HP left!")
                if (PlayerHP <= 0):
                    print("\tYou are too weak to move on, so you escape!")
                    break
        ...
        if Kills < reqA:
            print(enemy[0])
        elif Kills >= reqA and Kills < reqB:
            print(r.choice(enemy[0:-2]))
        elif Kills >= reqB and Kills < reqC:
            print(r.choice(enemy[0:-1]))
        elif Kills >= reqC:
            print(r.choice(enemy))
            break
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
        elif (a == "Use an Item"):
                A.print()
